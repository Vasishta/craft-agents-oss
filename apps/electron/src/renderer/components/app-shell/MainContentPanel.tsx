...
export function MainContentPanel({
  ...
  // Search navigator - global workspace search across docs and chats
  if (isSearchNavigation(navState)) {
    return wrapWithStoplight(
      <Panel variant="grow" className={className}>
        <SearchPage workspaceId={activeWorkspaceId || ''} />
      </Panel>
    )
  }

  // Outputs navigator - saved generated material awaiting review or promotion
  if (isOutputsNavigation(navState)) {
    return wrapWithStoplight(
      <Panel variant="grow" className={className}>
        {navState.details?.type === 'output' ? (
          <OutputDetailPage
            workspaceId={activeWorkspaceId || ''}
            outputId={navState.details.outputId}
          />
        ) : (
